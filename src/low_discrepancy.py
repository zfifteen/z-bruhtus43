...
class SamplerType(Enum):
    ...
    TANGENT_CHAIN = "tangent-chain"  # Tangent circle chain (φ-hierarchical)
...
class LowDiscrepancySampler:
    ...
    def __init__(self, sampler_type: SamplerType, dimension: int = 2,
        ...
        elif sampler_type == SamplerType.TANGENT_CHAIN:
            # Import here to avoid circular dependency
            try:
                from golden_ratio_tangent_circles import TangentChainSampler
                self.sampler = TangentChainSampler(base_radius=1.0, num_scales=5, seed=seed)
            except ImportError:
                raise ImportError("TangentChainSampler requires golden_ratio_tangent_circles module")
    ...
    def generate(self, n: int) -> np.ndarray:
        ...
        if self.sampler_type == SamplerType.PRNG:
        ...
        elif self.sampler_type == SamplerType.TANGENT_CHAIN:
            # Tangent chain sampler returns samples directly
            samples = self.sampler.generate_hierarchical_samples(n, dimension=self.dimension)
            # Normalize to [0, 1]^dimension if needed
            ...
            ranges[ranges == 0] = 1.0
            samples = (samples - min_vals) / ranges
            return samples
...
def demonstrate_low_discrepancy():
    ...
    samplers = [
        ...
        (SamplerType.TANGENT_CHAIN, "Tangent-chain (φ-hier)"),
